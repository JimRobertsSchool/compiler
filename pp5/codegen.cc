...
Location * CodeGenerator::arrayIndex(Location * addr, Location * index) {

	Location *zero, *four, *check1, *check2, *check3, *check, *length, *error, *offset, *loc, *ret;
	char * l1;

	zero = GenLoadConstant(0);
	check1 = GenBinaryOp("<", index, zero);

	length = GenLoad(addr, -4);

	check2 = GenBinaryOp("<", index, length);
	check3 = GenBinaryOp("==", check2, zero);
	check = GenBinaryOp("||", check1, check3);
	...
	GenLabel(l1);
	four = GenLoadConstant(4);

	offset = GenBinaryOp("*", four, index);

	loc = GenBinaryOp("+", addr, offset);
...
Location *CodeGenerator::GenTempVar()
{
  ...
  char temp[10];
  Node::tempNumb = tempNum = &nextTempNum;
  ...
  result = new Location(fpRelative, VarSize * (++stackNum + 1) * -1, temp);
  Assert(result != NULL);
